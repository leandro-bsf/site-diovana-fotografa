...
const ChartTooltipContent = React.forwardRef(({ active, payload, className, indicator = "dot", hideLabel = false, hideIndicator = false, label, labelFormatter, labelClassName, formatter, color, nameKey, labelKey, }, ref) => {
    ...
    const tooltipLabel = React.useMemo(() => {
        ...
        return labelFormatter ? (<div className={cn("font-medium", labelClassName)}>{labelFormatter(value, payload)}</div>) : value ? (<div className={cn("font-medium", labelClassName)}>{value}</div>) : null;
...
function getPayloadConfigFromPayload(config, payload, key) {
    if (typeof payload !== "object" || payload === null)
        return undefined;
    const p = payload;
    const payloadPayload = "payload" in p && typeof p.payload === "object" ? p.payload : undefined;
